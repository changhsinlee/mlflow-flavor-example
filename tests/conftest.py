"""Taken directly from https://github.com/mlflow/mlflow/blob/f7b97343dd6010b22915498d28838af14ccb0ff2/tests/conftest.py
with autologging fixtures removed.
"""
    ...
    try:
        if "notrackingurimock" not in request.keywords:
            tracking_uri = path_to_local_sqlite_uri(
                os.path.join(tmpdir.strpath, "mlruns.sqlite")
            )
...
def prevent_infer_pip_requirements_fallback(request):
    ...
    if "allow_infer_pip_requirements_fallback" not in request.keywords:
        with mock.patch(
            "mlflow.utils.environment._logger.exception", new=new_exception
        ):
            yield
    else:
        yield
